...
"""
function evolve(c::CurveProblem, solmethod = Tsit5; mdc_callback = CallbackCallable[],
        callback = nothing, saved_values = nothing, momentum_tol = 1e-3, kwargs...)
    ...
    sim = solve(e, Tsit5(), EnsembleThreads(), trajectories = length(probs);
